...
import json
import logging
from .models import Job, Resume, JobApplication, Notification
from .notifications import notify_application_approved, notify_application_rejected
from .forms import ResumeUploadForm, JobApplicationForm
...
def analytics_dashboard(request):
    ...
    context = {
        ...
        'total_resumes': total_resumes,
        'avg_score': avg_score,        'high_score_count': high_score_count,
...
def hr_applications(request):
    ...
    ).select_related('job', 'applicant', 'resume').order_by('-applied_at')
    
    return render(request, 'jobs/hr_applications.html', {
...
def hr_approved_applicants(request):
    ...
    jobs_with_approved = Job.objects.filter(
        posted_by=request.user,
        applications__status='approved'
    ).prefetch_related('applications__applicant').distinct()
    ...
    return render(request, 'jobs/approved_applicants.html', {
        'job_data': job_data
    })
