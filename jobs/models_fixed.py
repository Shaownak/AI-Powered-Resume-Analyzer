...
class Resume(models.Model):
    ...
    applicant = models.ForeignKey(
        User, on_delete=models.CASCADE, limit_choices_to={"user_type": "applicant"}, null=True, blank=True
    )
        ...
        try:
            ...
            with open(self.resume_file.path, "rb") as f:
                response = requests.post(
                    ...
                    files={"file": f},
                    data={"job_description": self.job.description, "async_mode": "true"},
                )
                ...
                if response.status_code == 200:
                    ...
                    if "task_id" in result:
                        ...
                        Resume.objects.filter(pk=self.pk).update(task_id=self.task_id, scoring_status="processing")
                        ...
                        Resume.objects.filter(pk=self.pk).update(score=self.score, scoring_status="completed")
    ...
    def _fallback_scoring(self):
        ...
        try:
            ...
            Resume.objects.filter(pk=self.pk).update(score=0.0, scoring_status="failed")
