...
from .models import ApplicantProfile, CustomUser, HRProfile
...
class ApplicantSignUpForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            field.widget.attrs["class"] = (
                "w-full px-4 py-3 border border-secondary-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200"
            )
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            ApplicantProfile.objects.create(user=user, full_name=self.cleaned_data["full_name"])
        return user
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            HRProfile.objects.create(
                user=user, company_name=self.cleaned_data["company_name"], job_title=self.cleaned_data.get("job_title", "")
            )
        return user
